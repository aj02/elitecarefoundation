...
function documentReadyInit() {
	...
	// Service carusel for .boxed layout
	...
	//Gallery item owl carousel
	if (jQuery().owlCarousel) {
		jQuery('.related-photos-carousel').each(function() {
			var $carousel = jQuery(this);
			var loop = $carousel.data('loop') ? $carousel.data('loop') : false;
			var margin = ($carousel.data('margin') || $carousel.data('margin') == 0) ? $carousel.data('margin') : 30;
			var nav = $carousel.data('nav') ? $carousel.data('nav') : false;
			...
			$carousel.owlCarousel({
				navContainer: ".gallery-single-slider-nav",
				loop: loop,
				margin: margin,
				nav: nav,
				autoplay: autoplay,
	...
	} //eof Gallery item owl carousel

	// Pages counter for Gallery item owl carousel
	var clonedItems = jQuery(".gallery-single-slider .cloned").size();
    jQuery('.related-photos-carousel').on('changed.owl.carousel', function(e) {
        if (!e.namespace || e.property.name != 'position') return
        jQuery('#related-photos-carousel-pages').text((e.item.index - clonedItems / 2 + 1) + ' / ' + e.item.count)
    })
...
function windowLoadInit() {
	...
	if ($header.length) {
		...
		jQuery($header).on('affixed-top.bs.affix', function () {
			$header.parent().css({
				...
				"margin-bottom": headerHeight - $header.outerHeight()
			});
		});
	...
	if (jQuery().jflickrfeed) {
		...
		jQuery("#side-flickr").jflickrfeed({
			...
			jQuery("#side-flickr a").prettyPhoto({
				hook: 'data-gal',
				theme: 'facebook'
	   		});
		});
...
function cleanDatepicker() {
	...
	$.datepicker._updateDatepicker = function(inst) {
		...
		$("<button type='button' class='ui-datepicker-clean ui-state-default ui-priority-primary ui-corner-all'>Clear</button>").appendTo(buttonPane).on('click', function(ev) {
...
jQuery(document).ready(function() {
	if(jQuery('#box_wrapper').hasClass('container')) {
		jQuery('#ui-datepicker-div').addClass("boxed");
	}
});

jQuery(document).ready(function() {
	if(jQuery('.contacts').hasClass('ds')) {
		jQuery('#ui-datepicker-div').addClass("ds");
	}
});
